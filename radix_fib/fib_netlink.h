 ...
 */
static void read_route(struct nlmsghdr *nh, int nll, radix_tree_t *f) {
...
//                                     *(struct in6_addr *) RTA_DATA(tb[RTA_DST])};
            struct net_addr_ip6 *a = (struct in6_addr *) RTA_DATA(tb[RTA_DST]);
            char ip[32], ip_prefix[32];
            inet_ntop(AF_INET6, a, ip, sizeof(ip));

            sprintf(ip_prefix, "%s/%d", ip, rt_msg->rtm_dst_len);
            node = make_node(f, ip_prefix, NULL);
            node->data = sid_node;
...
static int get_route_table(int rtm_family, radix_tree_t *f) {
    ...
    nll = recv_msg(sa, sock, buf, 4*1024*1024);
    if (nll < 0) {
...
void print_node(radix_node_t *node, void *data) {
    char ip[32];
    inet_ntop(AF_INET6, &node->prefix->add, ip, sizeof(ip));
    printf("%s\n", ip);
}

void fib6_print(radix_tree_t *f) {
    radix_process(f, print_node, NULL);
}
